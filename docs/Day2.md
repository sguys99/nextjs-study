...
이건 JS 면접 단골이자, `var`를 없애야 하는 결정적 이유입니다.
...
💡 왜 이런가: `var`는 함수 스코프라 루프 전체가 **변수 `i` 하나**를 공유합니다. `setTimeout`의 콜백이 나중에 실행될 땐 이미 `i`가 3이 돼 있죠. `let`은 반복마다 **새로운 바인딩**을 만들어서, 각 콜백이 그 순간의 값을 기억합니다. **이게 클로저가 "값을 복사"가 아니라 "변수를 참조"로 잡는다는 증거**입니다. (`setTimeout`은 다음 세션에서 자세히 — 지금은 "나중에 실행되는 함수" 정도로만.)

### 1-4. 고차함수 복습 + 콜백이라는 이름

Day 1에서 `map`/`filter`가 "함수를 인자로 받는 함수"라고 했죠. 그렇게 **인자로 넘겨지는 함수를 "콜백(callback)"**이라 부릅니다. 비동기의 출발점입니다.
...
### ✅ 세션 1 체크
- [ ] `let`/`const`는 블록 스코프, `var`는 함수 스코프임을 이해
- [ ] 클로저로 "비공개 상태를 가진 함수" 작성 (카운터 등)
- [ ] `var` 루프 함정과 `let`이 고치는 이유를 설명 가능
- [ ] 콜백 = "인자로 넘기는 함수"임을 인지
- [ ] "콜백은 화살표 함수로" 규칙 체득
