...
이미 프로그래밍에는 능숙한 분이죠. 그래서 Day 1의 목표는 "프로그래밍을 배우는 것"이 아니라 **"Python 습관을 JS 습관으로 번역하는 것"**이에요. 문법 자체보다 **함정(⚠️)**에 집중하세요.
...
**① 왜 있나**: 프로젝트 이름·버전·의존성·실행 스크립트를 한 곳에 적어두는 파일. 이게 있어야 남도 `pnpm install`로 똑같이 복원합니다.
...
**② 쉬운 설명**: `if`문 등에서 boolean이 아닌 값도 참/거짓으로 칩니다.
...
가장 많이 쓰는 5개를 차례로 봅니다. 공통점: **원본을 바꾸지 않고 새 배열/값을 반환**합니다(순수 함수).
...
`reduce`에 **초깃값이 없습니다.** 초깃값 없이 `reduce`를 쓰면 배열의 첫 원소를 초깃값으로 삼는데, `filter` 결과가 **빈 배열이면 `TypeError`**가 납니다("Reduce of empty array with no initial value"). 초깃값 `0`을 넣어 안전하게:
