...
🐍 오늘 쓰는 임베딩 모델 `all-MiniLM-L6-v2`는 **당신이 Python `sentence-transformers`로 써봤을 바로 그 모델**입니다. numpy로 하던 dot/norm 계산을 TS로 옮겨 적는 정도예요.
...
⚠️ **Next.js 설정 필수**: Transformers.js는 네이티브 모듈을 쓰기 때문에 Next.js가 번들링하려 하면 에러가 납니다. **서버 외부 패키지로 지정**하세요.
