...
1. **지도 그리기** — Python이라는 대륙은 훤히 알아도, JS/웹이라는 대륙은 처음입니다. 새 도구를 만날 때마다 "이게 Python의 무엇에 해당하지?"를 알면 학습 속도가 몇 배 빨라집니다.
...
문법을 배우기 전에 "왜 이런 언어가 됐는지"를 알면 뒤에서 만날 수많은 함정이 납득이 갑니다.
...
- 급하게 만들어진 데다 이미 퍼진 코드를 절대 깨면 안 됐기 때문에(하위호환), **이상한 부분을 고치지 못하고 계속 쌓아왔습니다.** `==`의 이상한 동작, `null`과 `undefined`가 둘 다 있는 것 등이 그 흔적이에요.
...
3. HTML 안에 "CSS 파일도 받아와", "JS 파일도 받아와"라는 링크가 있으면 브라우저가 그 파일들도 마저 받습니다.
4. 브라우저가 이 재료들을 조립해 **화면**을 그린 뒤, JS를 실행해 **움직임**을 붙입니다.

웹페이지는 결국 **HTML(구조) + CSS(꾸밈) + JavaScript(동작)** 이 셋을 합친 겁니다.
...
🐍 비유하자면 HTML은 **디스크에 있는 설정 파일(정적 텍스트)**, DOM은 그걸 파싱해서 **메모리에 올린 객체(런타임에 조작 가능한 상태)**입니다. 파일을 읽어 딕셔너리로 만든 뒤 그 딕셔너리를 수정하는 것과 비슷해요.
...
💡 지금 이 그림이 흐릿해도 괜찮습니다. Day가 지나면서 또렷해집니다. **오늘은 "웹 = HTML+CSS+JS, JS는 DOM으로 화면을 만진다, React는 그걸 대신 해준다"** 이 한 줄만 남으면 성공이에요.

---

## 3. 지형도 — Python vs JS 마스터 대비표

이제 도구 이야기입니다. 아래 표가 JS **생태계** 전체를 요약합니다.
...
설치 전에 Python 개발자가 특히 오해하기 쉬운 4개만 짚고 갑니다.
...
🟨 JS: 그런 거 없습니다. 폴더에서 `pnpm add`를 하면 **그 폴더의 `node_modules/`**에 깔리면서 폴더마다 자동으로 격리돼요. **activate 개념 자체가 없습니다.**
...
이제 실제로 설치합니다. **자기 OS 절만 따라가세요.** 각 단계 끝에 붙은 검증 명령은 꼭 실행하고 넘어갑니다.
...
설치 후 **터미널을 껐다 켜거나** 아래 명령으로 nvm을 로드하세요:
...
Day 1에서 본격적으로 설정하지만 지금은 확장 3개만 미리 깝니다.
...
앞으로 빨간 에러 메시지를 수백 번 봅니다. Python 개발자라면 미리 안심해도 좋은 점이 있습니다:

- JS/Node 에러도 **위에서부터** 읽습니다: 첫 줄에 "무엇이 잘못됐나"가 나오고 그 아래로 **스택 트레이스**(어디서 났나)가 이어집니다. 🐍 Python의 Traceback과 구조가 똑같아요.
- 흔한 3종:
